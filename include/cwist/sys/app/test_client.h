...
#define CWIST_ASSERT_STATUS(res, expected_status)                                            \
    do {                                                                                     \
        if (!(res) || (res)->status_code != (expected_status)) {                             \
            fprintf(stderr, "[ASSERT FAIL] %s:%d: expected status %d, got %d\n", __FILE__,   \
                    __LINE__, (int)(expected_status), (res) ? (int)(res)->status_code : -1); \
            exit(1);                                                                         \
        }                                                                                    \
    } while (0)

#define CWIST_ASSERT_HEADER(res, header_name, expected_value)                               \
    do {                                                                                    \
        ...
        if (!actual_val || strcmp(actual_val, (expected_value)) != 0) {                     \
            fprintf(stderr, "[ASSERT FAIL] %s:%d: expected header '%s' = '%s', got '%s'\n", \
                    __FILE__, __LINE__, (header_name), (expected_value),                    \
                    actual_val ? actual_val : "<null>");                                    \
            exit(1);                                                                        \
        }                                                                                   \
    } while (0)

#define CWIST_ASSERT_BODY_CONTAINS(res, snippet)                                               \
    do {                                                                                       \
