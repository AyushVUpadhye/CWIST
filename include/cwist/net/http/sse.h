...
#define CWIST_SSE_EVENT(payload) \
    ((cwist_sse_event_t){.event = NULL, .id = NULL, .retry_ms = -1, .data = (payload)})
#define CWIST_SSE_NAMED(event_name, payload) \
    ((cwist_sse_event_t){.event = (event_name), .id = NULL, .retry_ms = -1, .data = (payload)})
