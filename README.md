...
- **CWIST**: 117,024 req/s at 0.53ms average latency (P50 0.39ms, P90 1.04ms, P99 2.32ms)
- **Axum**: 119,835 req/s at 0.79ms average latency (P50 0.69ms, P90 1.45ms, P99 2.60ms), same binary as the main run above
...
- **CWIST (classic pool)**: 118816 req/s | Latency 1.97ms (P90 4.10ms, P99 7.67ms, P99.999 28.56ms) | RSS 16232KiB | Csw 0
- **CWIST (C1M reactor)**: 136628 req/s | Latency 3.31ms (P90 8.43ms, P99 16.15ms, P99.999 29.85ms) | RSS 11648KiB | Csw 0
- **CWIST (C1M reactor, arena_max=1)** — glibc arena cap adopted in PR #35 after mimalloc was tried and refuted (issue #25); this line confirms the decision on every run: 140371 req/s | Latency 3.03ms (P90 7.49ms, P99 14.22ms, P99.999 25.58ms) | RSS 12040KiB | Csw 0
- **CWIST (C1M reactor, drain_chunk=8)** — cooperative queuing for cwist_async_defer completions within a big io_uring batch (issue #25, docs/cooperative-queuing.md); this workload has no cwist_async_defer traffic to interleave, so parity with the plain C1M row above is the expected result, not a null finding — the tail-latency win is isolated directly in tests/bench_cooperative_queuing.c: 140248 req/s | Latency 3.03ms (P90 7.55ms, P99 14.40ms, P99.999 23.86ms) | RSS 11576KiB | Csw 0
- **Axum**: 110711 req/s | Latency 3.55ms (P90 5.97ms, P99 8.76ms, P99.999 17.34ms) | RSS 16724KiB | Csw 0
- **Gin (Go)**: 77093 req/s | Latency 7.30ms (P90 18.08ms, P99 40.76ms, P99.999 85.66ms) | RSS 30632KiB | Csw 0
- **Spring Boot**: 43866 req/s | Latency 9.08ms (P90 12.09ms, P99 18.73ms, P99.999 95.66ms) | RSS 1325892KiB | Csw 0
...
|---|---:|---:|---:|---:|---:|---:|
| AMD EPYC 7763 64-Core Processor | 52 | 2.01 | 2.73 | 3.52 | 121,104 | 111,302 |
| AMD EPYC 9V74 80-Core Processor | 25 | 1.86 | 2.37 | 3.24 | 127,922 | 120,629 |
