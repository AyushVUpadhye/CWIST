...
- **CWIST**: 191,250 req/s at 0.34ms average latency (P50 0.25ms, P90 0.59ms, P99 1.90ms)
- **Axum**: 195,491 req/s at 0.51ms average latency (P50 0.44ms, P90 0.88ms, P99 2.09ms), same binary as the main run above
...
- **CWIST (classic pool)**: 184710 req/s | Latency 1.27ms (P90 2.72ms, P99 5.60ms, P99.999 29.58ms) | RSS 16292KiB | Csw 0
- **CWIST (C1M reactor)**: 218093 req/s | Latency 2.32ms (P90 6.32ms, P99 13.16ms, P99.999 26.04ms) | RSS 10416KiB | Csw 0
- **CWIST (C1M reactor, arena_max=1)** — glibc arena cap adopted in PR #35 after mimalloc was tried and refuted (issue #25); this line confirms the decision on every run: 215875 req/s | Latency 2.34ms (P90 6.36ms, P99 13.35ms, P99.999 31.41ms) | RSS 11176KiB | Csw 0
- **CWIST (C1M reactor, drain_chunk=8)** — cooperative queuing for cwist_async_defer completions within a big io_uring batch (issue #25, docs/cooperative-queuing.md); this workload has no cwist_async_defer traffic to interleave, so parity with the plain C1M row above is the expected result, not a null finding — the tail-latency win is isolated directly in tests/bench_cooperative_queuing.c: 219562 req/s | Latency 2.36ms (P90 6.47ms, P99 13.49ms, P99.999 22.81ms) | RSS 8232KiB | Csw 0
- **Axum**: 177917 req/s | Latency 2.25ms (P90 4.08ms, P99 6.38ms, P99.999 12.12ms) | RSS 15728KiB | Csw 0
- **Gin (Go)**: 141960 req/s | Latency 3.68ms (P90 8.87ms, P99 18.80ms, P99.999 40.84ms) | RSS 29444KiB | Csw 0
- **Spring Boot**: 120188 req/s | Latency 3.28ms (P90 4.78ms, P99 6.97ms, P99.999 22.33ms) | RSS 1308196KiB | Csw 0
