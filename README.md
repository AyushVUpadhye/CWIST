...
- **CWIST (classic pool)**: 113320 req/s | Latency 2.05ms (P90 4.24ms, P99 8.29ms, P99.999 42.97ms) | RSS 17172KiB | Csw 0
- **CWIST (C1M reactor)**: 143176 req/s | Latency 3.05ms (P90 7.91ms, P99 14.71ms, P99.999 24.17ms) | RSS 10272KiB | Csw 0
- **CWIST (C1M reactor, arena_max=1)** — glibc arena cap adopted in PR #35 after mimalloc was tried and refuted (issue #25); this line confirms the decision on every run: 144556 req/s | Latency 2.96ms (P90 7.61ms, P99 14.35ms, P99.999 24.57ms) | RSS 12168KiB | Csw 0
- **CWIST (C1M reactor, drain_chunk=8)** — cooperative queuing for cwist_async_defer completions within a big io_uring batch (issue #25, docs/cooperative-queuing.md); this workload has no cwist_async_defer traffic to interleave, so parity with the plain C1M row above is the expected result, not a null finding — the tail-latency win is isolated directly in tests/bench_cooperative_queuing.c: 143731 req/s | Latency 3.10ms (P90 8.18ms, P99 15.70ms, P99.999 27.13ms) | RSS 9936KiB | Csw 0
- **Axum**: 121042 req/s | Latency 3.22ms (P90 5.48ms, P99 8.12ms, P99.999 18.26ms) | RSS 15148KiB | Csw 0
- **Gin (Go)**: 89834 req/s | Latency 5.73ms (P90 13.60ms, P99 29.32ms, P99.999 70.88ms) | RSS 29024KiB | Csw 0
- **Spring Boot**: 58012 req/s | Latency 6.82ms (P90 8.98ms, P99 14.40ms, P99.999 36.24ms) | RSS 1329220KiB | Csw 0
...
| AMD EPYC 9V74 80-Core Processor | 31 | 1.89 | 2.21 | 3.24 | 126,884 | 120,629 |
...
- **CWIST**: 121,527 req/s at 0.50ms average latency (P50 0.39ms, P90 0.97ms, P99 2.18ms)
- **Axum**: 124,569 req/s at 0.77ms average latency (P50 0.70ms, P90 1.27ms, P99 2.50ms), same binary as the main run above
