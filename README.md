...
- **CWIST (classic pool)**: 140294 req/s | Latency 1.63ms (P90 3.39ms, P99 6.49ms, P99.999 24.96ms) | RSS 16432KiB | Csw 0
- **CWIST (C1M reactor)**: 180266 req/s | Latency 2.65ms (P90 7.14ms, P99 13.92ms, P99.999 24.51ms) | RSS 9984KiB | Csw 0
- **CWIST (C1M reactor, arena_max=1)** — glibc arena cap adopted in PR #35 after mimalloc was tried and refuted (issue #25); this line confirms the decision on every run: 179282 req/s | Latency 2.73ms (P90 7.48ms, P99 14.47ms, P99.999 26.44ms) | RSS 11688KiB | Csw 0
- **CWIST (C1M reactor, drain_chunk=8)** — cooperative queuing for cwist_async_defer completions within a big io_uring batch (issue #25, docs/cooperative-queuing.md); this workload has no cwist_async_defer traffic to interleave, so parity with the plain C1M row above is the expected result, not a null finding — the tail-latency win is isolated directly in tests/bench_cooperative_queuing.c: 178407 req/s | Latency 2.63ms (P90 6.99ms, P99 13.89ms, P99.999 23.38ms) | RSS 10196KiB | Csw 0
- **Axum**: 149122 req/s | Latency 2.65ms (P90 4.67ms, P99 7.10ms, P99.999 13.52ms) | RSS 15744KiB | Csw 0
- **Gin (Go)**: 110461 req/s | Latency 5.25ms (P90 13.13ms, P99 30.42ms, P99.999 71.74ms) | RSS 29508KiB | Csw 0
- **Spring Boot**: 73504 req/s | Latency 5.47ms (P90 7.75ms, P99 15.50ms, P99.999 66.62ms) | RSS 1317712KiB | Csw 0
...
|---|---:|---:|---:|---:|---:|---:|
| AMD EPYC 7763 64-Core Processor | 44 | 2.01 | 2.58 | 3.51 | 119,152 | 111,377 |
...
- **CWIST**: 157,437 req/s at 0.40ms average latency (P50 0.31ms, P90 0.71ms, P99 2.03ms)
- **Axum**: 154,460 req/s at 0.64ms average latency (P50 0.55ms, P90 1.13ms, P99 2.42ms), same binary as the main run above
