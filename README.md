...
- **CWIST**: 192,158 req/s at 0.34ms average latency (P50 0.26ms, P90 0.59ms, P99 1.96ms)
- **Axum**: 204,304 req/s at 0.48ms average latency (P50 0.42ms, P90 0.81ms, P99 1.77ms), same binary as the main run above
...
- **CWIST (classic pool)**: 187586 req/s | Latency 1.26ms (P90 2.70ms, P99 5.98ms, P99.999 29.68ms) | RSS 16856KiB | Csw 0
- **CWIST (C1M reactor)**: 211392 req/s | Latency 2.70ms (P90 7.59ms, P99 16.67ms, P99.999 35.36ms) | RSS 11792KiB | Csw 0
- **CWIST (C1M reactor, arena_max=1)** — glibc arena cap adopted in PR #35 after mimalloc was tried and refuted (issue #25); this line confirms the decision on every run: 220366 req/s | Latency 2.34ms (P90 6.43ms, P99 13.35ms, P99.999 22.83ms) | RSS 14260KiB | Csw 0
- **CWIST (C1M reactor, drain_chunk=8)** — cooperative queuing for cwist_async_defer completions within a big io_uring batch (issue #25, docs/cooperative-queuing.md); this workload has no cwist_async_defer traffic to interleave, so parity with the plain C1M row above is the expected result, not a null finding — the tail-latency win is isolated directly in tests/bench_cooperative_queuing.c: 225017 req/s | Latency 2.30ms (P90 6.30ms, P99 13.24ms, P99.999 23.51ms) | RSS 9560KiB | Csw 0
- **Axum**: 194024 req/s | Latency 2.04ms (P90 3.67ms, P99 5.69ms, P99.999 11.93ms) | RSS 16344KiB | Csw 0
- **Gin (Go)**: 148865 req/s | Latency 3.46ms (P90 8.34ms, P99 16.99ms, P99.999 37.87ms) | RSS 30420KiB | Csw 0
- **Spring Boot**: 122456 req/s | Latency 3.27ms (P90 4.93ms, P99 7.83ms, P99.999 54.79ms) | RSS 1299628KiB | Csw 0
...
| AMD EPYC 9V74 80-Core Processor | 23 | 1.86 | 2.39 | 3.24 | 141,969 | 120,629 |
