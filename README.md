...
- **CWIST**: 152,586 req/s at 0.40ms average latency (P50 0.33ms, P90 0.67ms, P99 1.83ms)
- **Axum**: 161,125 req/s at 0.60ms average latency (P50 0.54ms, P90 0.99ms, P99 2.04ms), same binary as the main run above
...
- **CWIST (classic pool)**: 158371 req/s | Latency 1.47ms (P90 3.10ms, P99 6.01ms, P99.999 20.50ms) | RSS 16528KiB | Csw 0
- **CWIST (C1M reactor)**: 184966 req/s | Latency 2.59ms (P90 6.89ms, P99 13.83ms, P99.999 24.10ms) | RSS 11692KiB | Csw 0
- **CWIST (C1M reactor, arena_max=1)** — glibc arena cap adopted in PR #35 after mimalloc was tried and refuted (issue #25); this line confirms the decision on every run: 181531 req/s | Latency 2.56ms (P90 6.82ms, P99 13.26ms, P99.999 23.45ms) | RSS 11880KiB | Csw 0
- **CWIST (C1M reactor, drain_chunk=8)** — cooperative queuing for cwist_async_defer completions within a big io_uring batch (issue #25, docs/cooperative-queuing.md); this workload has no cwist_async_defer traffic to interleave, so parity with the plain C1M row above is the expected result, not a null finding — the tail-latency win is isolated directly in tests/bench_cooperative_queuing.c: 170461 req/s | Latency 3.12ms (P90 8.64ms, P99 18.17ms, P99.999 36.00ms) | RSS 9684KiB | Csw 0
- **Axum**: 153307 req/s | Latency 2.56ms (P90 4.49ms, P99 6.81ms, P99.999 13.33ms) | RSS 16096KiB | Csw 0
- **Gin (Go)**: 117950 req/s | Latency 4.29ms (P90 10.18ms, P99 21.23ms, P99.999 45.41ms) | RSS 29800KiB | Csw 0
- **Spring Boot**: 73771 req/s | Latency 5.35ms (P90 7.52ms, P99 11.11ms, P99.999 47.02ms) | RSS 1326840KiB | Csw 0
...
| AMD EPYC 9V74 80-Core Processor | 23 | 1.86 | 2.52 | 3.24 | 141,969 | 120,629 |
