...
- **CWIST**: 115,768 req/s at 0.54ms average latency (P50 0.44ms, P90 0.94ms, P99 2.30ms)
- **Axum**: 120,381 req/s at 0.79ms average latency (P50 0.68ms, P90 1.44ms, P99 2.60ms), same binary as the main run above
...
- **CWIST (classic pool)**: 115579 req/s | Latency 2.00ms (P90 4.09ms, P99 7.47ms, P99.999 20.51ms) | RSS 16692KiB | Csw 0
- **CWIST (C1M reactor)**: 136558 req/s | Latency 3.27ms (P90 8.32ms, P99 15.66ms, P99.999 42.21ms) | RSS 11564KiB | Csw 0
- **CWIST (C1M reactor, arena_max=1)** — glibc arena cap adopted in PR #35 after mimalloc was tried and refuted (issue #25); this line confirms the decision on every run: 135415 req/s | Latency 3.25ms (P90 8.28ms, P99 15.49ms, P99.999 27.34ms) | RSS 11732KiB | Csw 0
- **CWIST (C1M reactor, drain_chunk=8)** — cooperative queuing for cwist_async_defer completions within a big io_uring batch (issue #25, docs/cooperative-queuing.md); this workload has no cwist_async_defer traffic to interleave, so parity with the plain C1M row above is the expected result, not a null finding — the tail-latency win is isolated directly in tests/bench_cooperative_queuing.c: 136948 req/s | Latency 3.35ms (P90 8.69ms, P99 16.79ms, P99.999 33.97ms) | RSS 11492KiB | Csw 0
- **Axum**: 112201 req/s | Latency 3.50ms (P90 5.94ms, P99 8.86ms, P99.999 18.57ms) | RSS 16784KiB | Csw 0
- **Gin (Go)**: 78997 req/s | Latency 7.00ms (P90 17.26ms, P99 37.65ms, P99.999 87.75ms) | RSS 29568KiB | Csw 0
- **Spring Boot**: 43825 req/s | Latency 9.08ms (P90 12.10ms, P99 18.30ms, P99.999 83.86ms) | RSS 1320300KiB | Csw 0
...
|---|---:|---:|---:|---:|---:|---:|
| AMD EPYC 7763 64-Core Processor | 53 | 2.01 | 2.74 | 3.52 | 121,112 | 111,312 |
