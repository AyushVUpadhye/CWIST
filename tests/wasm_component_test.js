 ...
 *
 * Run by `make component-smoke` after the guest is built, componentized,
 * and transpiled into .jco-guest/. WASI imports come from
...
const JCO_DIR = path.resolve(process.cwd(), '.jco-guest');
...
(async () => {
  ...
  let failed = false;
  try {
    component.dispatch(new Uint8Array([0x00, 0xff, 0x00]));
  } catch (e) {
    failed = true;
  }
