...
static size_t encoded_varint_size(const uint8_t *buf) {
    return (size_t)1 << (buf[0] >> 6);
}

static uint64_t decode_varint(const uint8_t *buf, size_t *used) {
    size_t i;
    size_t size = encoded_varint_size(buf);
    uint64_t value = buf[0] & 0x3f;

    for (i = 1; i < size; i++) value = (value << 8) | buf[i];

    if (used) *used = size;
    return value;
}

static int test_initialization_and_clamping(void) {
    quic_conn_fc_t fc;
    ...
    quic_flow_control_init(&fc, 2 * MIN_WINDOW, 3 * MIN_WINDOW, MIN_WINDOW, MIN_WINDOW);
...
static int test_rtt_quality(void) {
    quic_conn_fc_t fc;

    quic_flow_control_init(&fc, MIN_WINDOW, MIN_WINDOW, 4 * MIN_WINDOW, 4 * MIN_WINDOW);
...
static int test_window_adjustment(void) {
    quic_conn_fc_t fc;
    quic_stream_fc_t sfc;

    quic_flow_control_init(&fc, MIN_WINDOW, MIN_WINDOW, 3 * MIN_WINDOW, 3 * MIN_WINDOW);
...
static int test_consumption_and_max_data_frames(void) {
    quic_conn_fc_t fc;
    uint8_t buf[16];
    size_t written = 99;
    size_t used;
    ...
    CHECK(quic_flow_control_maybe_send_max_data(&fc, buf, sizeof(buf), &written, false) == 0);
    CHECK(written == 0);
    CHECK(buf[0] == 0xa5);

    quic_flow_control_consume_conn(&fc, MIN_WINDOW / 2);
    CHECK(fc.conn_consumed == MIN_WINDOW / 2);
    CHECK(quic_flow_control_maybe_send_max_data(&fc, buf, sizeof(buf), &written, false) == 1);
    CHECK(buf[0] == 0x10);
    CHECK(written == 5);
    ...
    CHECK(quic_flow_control_maybe_send_max_data(&fc, buf, sizeof(buf), &written, true) == 1);
    ...
    written = 7;
    CHECK(quic_flow_control_maybe_send_max_data(NULL, buf, sizeof(buf), &written, false) == -1);
    CHECK(written == 0);
    CHECK(quic_flow_control_maybe_send_max_data(&fc, NULL, sizeof(buf), &written, false) == -1);
    CHECK(quic_flow_control_maybe_send_max_data(&fc, buf, sizeof(buf), NULL, false) == -1);
    quic_flow_control_consume_conn(NULL, 1);

    return 0;
}

static int test_max_stream_data_frames(void) {
    ...
    for (i = 0; i < sizeof(ids) / sizeof(ids[0]); i++) {
        ...
        CHECK(quic_flow_control_maybe_send_max_stream_data(&fc, &sfc, buf, sizeof(buf), &written,
                                                           true) == 1);
        CHECK(buf[0] == 0x11);
    ...
    CHECK(quic_flow_control_maybe_send_max_stream_data(&fc, &sfc, buf, 5, &written, true) == -1);
    CHECK(written == 0);
    ...
    written = 1;
    CHECK(quic_flow_control_maybe_send_max_stream_data(NULL, &sfc, buf, sizeof(buf), &written,
                                                       false) == -1);
    CHECK(written == 0);
    CHECK(quic_flow_control_maybe_send_max_stream_data(&fc, NULL, buf, sizeof(buf), &written,
                                                       false) == -1);
    CHECK(quic_flow_control_maybe_send_max_stream_data(&fc, &sfc, NULL, sizeof(buf), &written,
                                                       false) == -1);
    CHECK(quic_flow_control_maybe_send_max_stream_data(&fc, &sfc, buf, sizeof(buf), NULL, false) ==
          -1);
