...
static void assert_window_update(const uint8_t *frame, uint32_t stream_id, uint32_t increment) {
    assert(frame[0] == 0);
    assert(frame[1] == 0);
    assert(frame[2] == 4);
    assert(frame[3] == 0x08);
...
static void test_initialization_and_rtt(void) {
    ...
    cwist_http2_flow_control_init(&connection, 2U * 1024U * 1024U, 8U * 1024U * 1024U);
...
static void test_receive_consume_and_connection_window_update(void) {
    ...
    assert(cwist_http2_flow_control_maybe_window_update(&connection, frame, sizeof(frame), &written,
                                                        100, false) == 0);
    assert(written == 0);
    ...
    cwist_http2_flow_control_consume(&connection, 1U);
    assert(cwist_http2_flow_control_maybe_window_update(&connection, frame, FRAME_SIZE - 1U,
                                                        &written, 101, true) == -1);
    assert(written == 0);
...
static void test_connection_auto_tuning(void) {
    ...
    cwist_http2_flow_control_init(&connection, CWIST_HTTP2_INITIAL_CONNECTION_WINDOW,
    ...
    assert(connection.pacing_rate_bytes_per_sec == (uint64_t)initial * 1000000U / 100U);
...
static void test_stream_window_update(void) {
    ...
    cwist_http2_flow_control_init(&connection, 0, CWIST_HTTP2_INITIAL_CONNECTION_WINDOW * 2U);
    cwist_http2_flow_control_update_rtt(&connection, 100);
    connection.last_window_update_us = 1000;

    cwist_http2_stream_flow_control_init(&stream, 7, CWIST_HTTP2_INITIAL_STREAM_WINDOW,
    ...
    cwist_http2_stream_flow_control_init(&invalid_stream, CWIST_HTTP2_MAX_WINDOW + 1U, 0, 0);
    cwist_http2_stream_flow_control_consume(&invalid_stream, 1U);
    assert(cwist_http2_stream_flow_control_maybe_window_update(
               &connection, &invalid_stream, frame, sizeof(frame), &written, 1100, true) == 0);
}
...
static void test_send_windows_and_pacing(void) {
    ...
    allowance = cwist_http2_flow_control_pacing_allowance(&connection, &stream, 2000, 10);
