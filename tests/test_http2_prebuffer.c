...
static const unsigned char flow_flight[] = "PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n"
...
static void *serve(void *opaque) {
    struct server *s = opaque;
    char seed[256];
    cwist_https_connection conn = {.fd = s->fd,
                                   .ssl = NULL,
                                   .read_buf = seed,
                                   .buf_len = 0,
...
static int responses(int fd, int64_t end, const char *name, unsigned streams) {
    ...
    while (!r[0].ended || (streams == 2 && !r[1].ended) || !settings || !settings_ack) {
        ...
        uint32_t id =
            ((uint32_t)(h[5] & 127) << 24) | ((uint32_t)h[6] << 16) | ((uint32_t)h[7] << 8) | h[8];
        ...
        if (h[3] == 6) {
            if (id || n != 8) goto protocol_error;
            continue;
        }
        if (h[3] == 8) {
            if (n != 4) goto protocol_error;
            continue;
        }
    ...
    for (unsigned i = 0; i < streams; ++i) {
        if (!r[i].headers || r[i].used != strlen(bodies[i]) ||
            memcmp(r[i].body, bodies[i], r[i].used) != 0)
            goto protocol_error;
    }
    return 0;
io_error:
    fprintf(
        stderr,
        "%s: response read: %s (stream1 headers=%d bytes=%zu end=%d; stream3 headers=%d bytes=%zu end=%d)\n",
        name, strerror(errno), r[0].headers, r[0].used, r[0].ended, r[1].headers, r[1].used,
        r[1].ended);
    return -1;
protocol_error:
...
static int run_case(const char *name, const unsigned char *payload, size_t len, size_t seed,
                    unsigned streams) {
    ...
    if (transfer(fd[0], sendbuf, len, 1, s.deadline) != 0) {
        perror("request flight");
        goto done;
    }
...
int main(int argc, char **argv) {
    ...
    if (argc == 1 || strcmp(argv[1], "basic") == 0) {
        ...
        failed |=
            run_case("split first HEADERS header", flight, sizeof(flight) - 1, 24 + 9 + 5, 2) != 0;
        failed |= run_case("all requests prebuffered", flight, sizeof(flight) - 1,
                           sizeof(flight) - 1, 2) != 0;
    }
    if (argc == 1 || strcmp(argv[1], "flow") == 0) {
        failed |= run_case("flow-control socket control", flow_flight, sizeof(flow_flight) - 1, 0,
                           1) != 0;
        failed |= run_case("flow-control update prebuffered", flow_flight, sizeof(flow_flight) - 1,
                           sizeof(flow_flight) - 1, 1) != 0;
    }
    alarm(0);
